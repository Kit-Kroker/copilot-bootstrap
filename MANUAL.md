...
| 11 | `generate_hooks` | `generate-brownfield-hooks` | .vscode/settings.json |
| 12 | `generate_project_agent` | `generate-project-agent` | .github/agents/project.agent.md |
...
### Project *(brownfield — generated by `/generate`)*
**File:** `.github/agents/project.agent.md` | **Visible in chat:** Yes

This agent is **not part of the bootstrap pipeline** — it is an output of the pipeline. Generated by the `generate-project-agent` skill as the final step of `/generate`, it becomes the primary development agent once bootstrap is complete.

The generated agent is fully tailored to the discovered project: it references actual capability IDs (BC-001, BC-002...), real entity names, the detected tech stack, the code paths from `docs/discovery/domain-model.md`, and the project-specific skills generated alongside it. Developers select this agent in Copilot Chat to start feature work after the discovery phase.

After running `/finish`, this is the only agent that remains in `.github/agents/`.
...
#### `/generate`
Generate project-specific Copilot configuration from discovery outputs. Creates `.discovery/generate.lock.json` and runs 5 steps: copilot instructions, dev skills, project prompts, hooks, and the project agent.
...
- `generate-brownfield-hooks` — PostToolUse hooks in `.vscode/settings.json` wiring the detected linter and formatter to run automatically after file edits
